...
output = cmd(['./context_switch', str(num)])
plot = PeriodPlot()
...
for line in output.splitlines():
  ...
  match = re.match('(PID: )(.*)(, )(Active|Inactive)(.*)(: start at )(.*)(, duration )(.*)( cycles \()(.*)(ms\))', line)  #match = re.match('(.*:)(start at)(.*)(,)(.*)(,)(\(.*ms\))', line) 
  ...
  time = float(match.groups()[10]) 
  if match.groups()[3] == "Inactive":
    color = "red"
  else:
    color = "blue" #active period - blue rectangluar 
  plot.AddPeriod(startpoint, startpoint + time, color);
  startpoint+=time
plot.CreatePlot('activity periods', 'context_switch.eps')
