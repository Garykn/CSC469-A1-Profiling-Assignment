...
/*averageActive holds the average activity period of a thread*/
long double averageActive;

/*averageInActive holds the average inactivity period of a thread*/
long double averageInActive;

/*threshold holds the number of cycles in 1000ns*/
u_int64_t threshold;
 ...
 */
static u_int64_t inactive_periods(int num_periods, u_int64_t threshold, u_int64_t *samples) {
...
void* activityFunc(void * num_periods) {
	int n_periods = *(int*)num_periods;
	u_int64_t *samples = ( u_int64_t *)malloc(sizeof(uint64_t) * n_periods * 2);
	assert(samples != NULL);
	
	//wait for the other thread before starting to collect activity information 
	pthread_barrier_wait(&mybarrier);
	
	//get activity periods
	u_int64_t active_start_point = inactive_periods(n_periods, threshold, samples);
	
	
	/*print activity periods and compute their average in us*/	
	int i;
	averageActive=0;
	for(int i = 0; i < n_periods; i++) {
		/* print active periods*/
	 
		printf("Thread ID: %d, Active %d: start at %llu, duration %llu cycles (%.6Lf us)\n", pthread_self(), i, active_start_point, 
		(samples[2*i] - active_start_point), (samples[2*i] - active_start_point)/(long double)(cycles_per_ms/1E3)); 
		
		if(i > 0 && i < n_periods - 1){
	...
	averageActive = averageActive/(n_periods - 2);
...
//define thread functions
void* inActivityFunc(void * num_periods) {
	int n_periods = *(int*)num_periods;
	u_int64_t *samples = ( u_int64_t *)malloc(sizeof(uint64_t) * n_periods * 2);
	assert(samples != NULL);
	...
	/*print inactivity periods and compute their average in us*/	
	int i;
	averageInActive=0;
	for(int i = 0; i < n_periods; i++) {
		/* print inactive periods*/
	 
		printf("thread ID: %d, Inactive %d: start at %llu, duration %llu cycles (%.6Lf ms)\n", pthread_self(), i, samples[2*i], 
				(samples[2*i+1] - samples[2*i]), (samples[2*i+1] - samples[2*i])/(long double)(cycles_per_ms/1E3));
	...
	//compute average inactive period
	averageInActive = averageInActive/(n_periods - 2);
...
int main(int argc, char const *argv[]){	
	...
	threshold = cycles_per_ms / 1E3; //number of cycles in 1000ns
	
	//bind our process to CPU #3. Spawned pthreads will be bound to the same processor
	...
	/*spawn 2 threads and join on them*/
	 pthread_t ids[THREAD_COUNT];
	
	//initalize counting barrier to thread count
	pthread_barrier_init(&mybarrier, NULL, THREAD_COUNT + 1);
	
	//create threads
    pthread_create(&ids[0], NULL, inActivityFunc, &num_periods);
	pthread_create(&ids[1], NULL, activityFunc, &num_periods);
	...
	//compute context switch time between the two pthreads
