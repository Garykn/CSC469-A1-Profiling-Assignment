...
int main(int argc, char const *argv[]){
	...
	//Raise the priority of this process so that the process and its children are always selectied by the schedule when runnable
    //setpriority(PRIO_PROCESS, 0, -20);
	
	//bind our process to CPU #3. CPU affinity is inherited by child processes.
	...
	/*fork a child. 
	Going by the fact that if two processes are exculisvely running on the same idle/low-activity CPU, then we can quantify an inactive period of one 
	process to be the active period of the other process + 2 context swtiches. Thus, if we compute the average active period of one process
	and the average inactive period of the other process, then the difference between these two averages would be a reasonable estimate 
	of 2 context switch times. 
	 */
	pid_t pid = fork();

	/*
	The child process prints it's activite and inactive periods and computes the average of its active periods.
	*/
    if(pid == 0){	//child	
		...
		//allocate memory to store activity information
        u_int64_t *samples = ( u_int64_t *)malloc(sizeof(uint64_t) * num_periods * 2);
		assert(samples != NULL);
		
		/* block waiting on the other process so that both process can start recording the activity periods together. This way, 
		the acitivy period of one process corresponds to an inactivity period of the other process + 2 context switches*/
		start_now++;
		while(start_now != 2);
		
		/* Collect samples of activity periods */
        u_int64_t active_start_point = inactive_periods(num_periods, threshold, samples);

		/*print activity periods and compute their average*/	
		int i;
		long double averageActive=0;
		for(int i = 0; i < num_periods; i++) {
			/* print each active period followd by an inactive periods. These periods alternate in succession.
			Also, compute the average active period in ms.*/
		 
			printf("PID: %d, Active %d: start at %llu, duration %llu cycles (%.6Lf ms)\n", getpid(), i, active_start_point, 
			(samples[2*i] - active_start_point), (samples[2*i] - active_start_point)/(long double)cycles_per_ms); 
			
			if(i > 0 && i < num_periods - 1){
				averageActive += (samples[2*i] - active_start_point)/(long double)cycles_per_ms;
			}
			
		    printf("PID: %d, Inactive %d: start at %llu, duration %llu cycles (%.6Lf ms)\n", getpid(), i, samples[2*i], 
		    (samples[2*i+1] - samples[2*i]), (samples[2*i+1] - samples[2*i])/(long double)cycles_per_ms); 
			
			//update active_start_point to point to the end point of the current inactive period.
			active_start_point = samples[2*i+1]; //next active period starts when this inactive period ends
		}
		
		//compute average active period
		averageActive = averageActive/(num_periods - 2);
		
		//send averageActive to parent through pipe fd 
		write(fd[1], &averageActive, sizeof(averageActive));
		...
		//allocate memory to store acitivty periods
        u_int64_t *samples = ( u_int64_t *)malloc(sizeof(uint64_t) * num_periods * 2);
		assert(samples != NULL);
		
		// block waiting on the other process. 
		start_now++;
		while(start_now != 2);
        /* Collect samples of activity periods for the parent*/
		u_int64_t active_start_point = inactive_periods(num_periods, threshold, samples);
		
		/* print activity periods and compute the average of inacitivty periods */	
		int i;
		long double averageInActive=0; //average inactive period 
		for( i = 0; i < num_periods; i++) {
			...
			if(i > 0 && i < num_periods - 1){
				averageInActive += (samples[2*i+1] - samples[2*i])/(long double)cycles_per_ms;
			}
			//update active_start_point to point to the end point of the current inactive period.
			active_start_point = samples[2*i+1];
		}
		
		//compute the average inactive period
		averageInActive = averageInActive/(num_periods - 2);
		
		//read averageActive from child through pipe 
		long double averageActive;
	
		if( read(fd[0], &averageActive, sizeof(averageActive)) != sizeof(averageActive)){
		...
		//compute context switch time
		long double context_Switch_time = (averageInActive - averageActive)/2;
		
		printf("The context switch time is: %.6Lfms\n", context_Switch_time);
		return 0;
	}
}